...
(0, config_env_1.extendEnvironment)((hre) => {
    ...
    if ((hre.config.solidity.compilers.length > 1 && hre.config.resolc.compilerSource === "npm") ||
        ...
        throw new errors_1.ResolcPluginError(`Multiple solidity versions are not available when using npm as the compiler.`);
    }
...
(0, config_env_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE_SOLC, async (args, hre, runSuper) => {
    ...
    const output = await hre.run(task_names_1.TASK_COMPILE_SOLIDITY_RUN_SOLC, {
        input: args.input,
